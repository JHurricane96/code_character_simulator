...
string ActorTypeToString(ActorType type) {
	switch(type) {
		case ActorType::MAGICIAN		: return "magician";
		case ActorType::FIREBALL		: return "fire_ball";
...
pair<State, vector<vector<shared_ptr<Actor> > > > MakeState(Terrain terrain) {
	...
	for (int i = 6; i < 26; i++) {
		if (i < 10) {
			...
			Actor* actor = new Magician(i, PLAYER2, 50, 100, 100, 10, 10, 0, 0, 0, Vector2D(99, 9), Vector2D(0, 0), 1000, 10, 1000, 10, 300, 10);
			AddActor(arch, actor);
		} else if (i < 17) {
            Actor* actor =
