...
pair<State, vector<vector<shared_ptr<Actor> > > > MakeState(Terrain terrain) {
	...
	State S(terrain, sorted_actors, kings, bases, flags, towers, archers);
...
int main(int argc, char const* argv[])
{
	...
	auto S1 = thing1.first;
	PlayerStateHandler PSH1(&S1, PLAYER1);
	auto sorted_actors1 = thing1.second;
	auto actors1 = GetFlatActors(sorted_actors1);

	auto thing2 = MakeState(TT);
	auto S2 = thing2.first;
	PlayerStateHandler PSH2(&S2, PLAYER2);
	...
	PSH2.MoveUnits(list_act_id_t({3}), PSH2.GetEnemyFlag().GetPosition(), F, weights, NULL);
	S2.Update(1);
	// PrintLos(S2.GetTerrain(), PLAYER2);
	// cout << "TOWERS" << PSH2.GetEnemyTowers().size() << endl;
	PSH2.MoveUnits(s, PSH2.GetEnemyTowers()[3].GetPosition(), F, weights, NULL);

	S.MergeWithBuffer(S2, PLAYER2);
	...
	while (true) {
		...
		S.MergeWithBuffer(S1, PLAYER1);
		S.MergeWithBuffer(S2, PLAYER2);
		S.Update(0.8);
		S1.MergeWithMain(S);
		S2.MergeWithMain(S);
		auto pos = PSH2.GetEnemyFlag().GetPosition();
		cout << endl << pos.x << ' ' << pos.y << endl;
		if(!flag_captured) {
			unique_ptr<int> flag_success(new int());
			PSH2.FlagCapture(flag_success.get());
			if (*flag_success == 1 || *flag_success == (-2)) {
				cout << "Flag Captured" << endl;
				PSH2.MoveUnits(list_act_id_t({3}), PSH2.GetFlag().GetPosition(), F, weights, NULL);
				flag_captured = true;
			}
		}

		if (!done) {
			unique_ptr<int> attack_success(new int());
			// PSH1.AttackUnit(l, 3, attack_success.get());
			// S.MoveUnits(vector<int64_t>({3}), Vector2D(19 * te_size, 19 * te_size), F, weights);
			// cout << "WOLOLOLOLOLOLOLOLOLOLOLOLOLOLOLOLOLOLO" << *attack_success << endl;
			done = true;
		}

		unique_ptr<int> success(new int());
		PSH2.AttackUnit(s, 8, success.get());
		// cout << "SUCCESS: " << *success << ' ' << actors[8]->GetHp() << " ";
		// cout << ((actors[20]->GetAttackTarget() == nullptr) ? "not attacking" : "attacking") << endl;
