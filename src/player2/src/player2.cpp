...
void Player2::Update(std::shared_ptr<state::PlayerStateHandler> state)
{
	...
	if (balls == 0) {
		...
		state->MoveUnits(state::list_act_id_t(sws.begin(), sws.begin() + 4),
			state->GetEnemyKing(NULL).GetPosition(), f.get(),
			std::vector<int64_t>(3, 1), crap, NULL);
		state->PlanPath(magicians.begin()->GetPosition(),
			physics::Vector2D(300, 300), std::vector<int64_t>({ 1, 1, 1 }),
			path, NULL);
		...
		// NULL);
		// state->MoveUnits(sword_ids, physics::Vector2D(99 * 2, 99 * 2),
		// f.get(), std::vector<int64_t>(3, 1), NULL);
		// std::cout << "Soorya is a cuck!: " <<
		// *(state->GetPlayerEnemyIds().begin()) << "\n";
	...
	// }
	// if (kingthing == 0) {
	// 	state->MoveUnits(list_act_id_t({3}),
	// state->GetEnemyFlag().GetPosition(), f.get(), std::vector<int64_t>(3, 1),
	// NULL);
	// 	kingthing = 1;
	// }
	// else if (kingthing == 1 &&
	// !state->GetKing().GetPathPlannerHelper().IsPathPlanning()) {
	// 	state->FlagCapture(NULL);
	// 	state->MoveUnits(list_act_id_t({3}), state->GetBase().GetPosition(),
	// f.get(), std::vector<int64_t>(3, 1), NULL);
	// 	kingthing = 2;
	// }
	// else if (kingthing == 2 &&
	// !state->GetKing().GetPathPlannerHelper().IsPathPlanning()) {
	// 	state->FlagDrop(NULL);
	// 	state->MoveUnits(list_act_id_t({3}), physics::Vector2D(200, 40),
	// f.get(), std::vector<int64_t>(3, 1), NULL);
	// 	kingthing = 3;
	// }
